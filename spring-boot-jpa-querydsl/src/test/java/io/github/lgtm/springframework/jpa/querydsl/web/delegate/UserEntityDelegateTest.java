...
class UserEntityDelegateTest {
  ...
  void testGetController() {
    ...
    assertEquals(HttpStatus.OK, response.getStatusCode());
    assertNotNull(response.getBody());
    assertEquals("delegate-user", response.getBody().getName());
  }

  @TestConfiguration
  static class UserEntityDelegateConfiguration {
    @Bean
    public UserEntityDelegate userEntityDelegate() {
      return new UserEntityDelegate();
    }
  }
}
